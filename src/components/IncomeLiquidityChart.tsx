...
export function IncomeLiquidityChart() {
  ...
  const calculateEffectiveLiquidity = () => {
    ...
    incomeSources.forEach(source => {
      ...
      // Find the liquidity percentage for this income type
  ...
  const partnerBreakdown = partners.map(partner => {
    ...
    return {
      name: partner.name.split(' ')[0], // First name only
      ...breakdown,
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="p-3 rounded-lg bg-wealth-positive/10 border border-wealth-positive/20">
                ...
                <p className="text-lg font-bold font-mono">{formatCurrency(liquidityTotals.L1)}</p>
              </div>
              <div className="p-3 rounded-lg bg-accent/10 border border-accent/20">
                ...
                <p className="text-lg font-bold font-mono">
                  {formatCurrency(liquidityTotals.L3 + liquidityTotals.NL)}
                </p>
              </div>
            </div>

            {/* Effective Liquid Amount */}
            <div className="p-4 rounded-lg bg-primary/10 border border-primary/20">
              <div className="flex items-center justify-between mb-2">
                <span className="text-sm">Total Monthly Income</span>
                <span className="font-mono font-medium">{formatCurrency(effectiveLiquidity.totalIncome)}</span>
              </div>
              <div className="flex items-center justify-between mb-2">
                <span className="text-sm">Effective Liquid Amount</span>
                <span className="font-mono font-medium text-primary">{formatCurrency(effectiveLiquidity.liquidIncome)}</span>
              </div>
            ...
            {partnerBreakdown.length > 0 && (
              <div className="space-y-2">
                <p className="text-xs text-muted-foreground uppercase tracking-wider">By Partner</p>
                {partnerBreakdown.map(partner => (
                  <div key={partner.name} className="flex items-center gap-3">
                    <span className="text-sm w-20 truncate">{partner.name}</span>
                      ...
                      {(['L1', 'L2', 'L3', 'NL'] as const).map(level => {
                        ...
                        return (
                          <div 
                            ...
                            style={{ 
                              width: `${width}%`, 
                              backgroundColor: LIQUIDITY_COLORS[level] 
                            }}
                          />
                        );
                      })}
                    </div>
                    <span className="text-sm font-mono w-24 text-right">{formatCurrency(partner.total)}</span>
                  </div>
                ))}
              </div>
            )}
